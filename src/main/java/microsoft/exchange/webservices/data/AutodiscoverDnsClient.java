...
class AutodiscoverDnsClient {
    ...
    protected String findAutodiscoverHostFromSrv(String domain) throws XMLStreamException, IOException {
        ...
        if ((dnsSrvRecord == null) || dnsSrvRecord.getNameTarget() == null ||
            ...
            this.service.traceMessage(TraceFlags.AutodiscoverConfiguration,
                    String.format("DNS query for SRV record for domain %s found %s", domain,
                            dnsSrvRecord.getNameTarget()));
    ...
    private DnsSrvRecord findBestMatchingSrvRecord(String domain) throws XMLStreamException, IOException {
        ...
        try {
            ...
            String dnsExcMessage =
                    String.format("DnsQuery returned error " + "error '%s' error code 0x{1:X8}.", ex.getMessage(),
                            ex.getError());
            ...
            // unmanaged code.
            this.service.traceMessage(TraceFlags.AutodiscoverConfiguration,
                    String.format("DnsQuery cannot be called. Security error: %s.", ex.getMessage()));
            return null;
        }

        this.service.traceMessage(TraceFlags.AutodiscoverConfiguration,
                String.format("%d SRV records were returned.", dnsSrvRecordList.size()));
        ...
        // randomly pick one.
        int recordIndex =
                (bestDnsSrvRecordList.size() > 1) ? RandomTieBreakerSelector.nextInt(bestDnsSrvRecordList.size()) : 0;
        ...
        String traceMessage = String.format("Returning SRV record %d " +
                "of %d records. " +
                "Target: %s, Priority: %d, Weight: %d", recordIndex, dnsSrvRecordList.size(),
                bestDnsSrvRecord.getNameTarget(), bestDnsSrvRecord.getPriority(), bestDnsSrvRecord.getWeight());
