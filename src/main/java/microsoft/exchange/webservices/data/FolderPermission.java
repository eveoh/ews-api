...
public final class FolderPermission extends ComplexProperty implements IComplexPropertyChangedDelegate {
    ...
    private static LazyMember<List<FolderPermission>> levelVariants =
            new LazyMember<List<FolderPermission>>(new ILazyMember<List<FolderPermission>>() {
                @Override
    ...
    /**
     * * Determines whether the specified folder permission is the same as this one. The comparison does not take UserId
     * and PermissionLevel into consideration.
     *
     ...
     */
    void validate(boolean isCalendarFolder, int permissionIndex) throws ServiceValidationException,
    ...
    protected boolean tryReadElementFromXml(EwsServiceXmlReader reader) throws Exception {
        ...
        else if (reader.getLocalName().equalsIgnoreCase(XmlElementNames.PermissionLevel) ||
                reader.getLocalName().equalsIgnoreCase(XmlElementNames.CalendarPermissionLevel)) {
     ...
     * @throws Exception the exception
     */
    protected void writeToXml(EwsServiceXmlWriter writer, String xmlElementName, boolean isCalendarFolder) throws
            Exception {
