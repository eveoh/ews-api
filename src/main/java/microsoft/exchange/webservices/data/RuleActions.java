...
public final class RuleActions extends ComplexProperty {
    ...
    /**
     * Gets the categories that should be stamped on incoming messages. To disable stamping incoming messages with
     * categories, set AssignCategories to null.
     */
    ...
    /**
     * Gets or sets the Id of the folder incoming messages should be copied to. To disable copying incoming messages
     * to a
     * folder, set CopyToFolder to null.
     */
    ...
    /**
     * Gets or sets a value indicating whether incoming messages should be automatically moved to the Deleted Items
     * folder.
     */
    ...
    /**
     * Gets the e-mail addresses to which incoming messages should be forwarded as attachments. To disable forwarding
     * incoming messages as attachments, empty the ForwardAsAttachmentToRecipients list.
     */
    ...
    /**
     * Gets the e-mail addresses to which incoming messages should be forwarded. To disable forwarding incoming
     * messages,
    ...
    /**
     * Gets or sets the importance that should be stamped on incoming messages. To disable the stamping of incoming
     * messages with an importance, set MarkImportance to null.
     */
    ...
    /**
     * Gets or sets the Id of the folder to which incoming messages should be moved. To disable the moving of incoming
     * messages to a folder, set CopyToFolder to null.
     */
    ...
    /**
     * Gets or sets a value indicating whether incoming messages should be permanently deleted. When a message is
     * permanently deleted, it is never saved into the recipient's mailbox. To delete a message after it has saved
     * into the
     * recipient's mailbox. To delete a message after it has
     */
    ...
    /**
     * Gets the e-mail addresses to which incoming messages should be redirecteded. To disable redirection of incoming
     * messages, empty the RedirectToRecipients list. Unlike forwarded mail, redirected mail maintains the original
     * sender
     * and recipients.
     */
    ...
    /**
     * Gets the phone numbers to which an SMS alert should be sent. To disable sending SMS alerts for incoming messages,
     * empty the SendSMSAlertToRecipients list.
     */
    ...
    /**
     * Gets or sets the Id of the template message that should be sent as a reply to incoming messages. To disable
     * automatic replies, set ServerReplyWithMessage to null.
     */
    ...
    protected void writeElementsToXml(EwsServiceXmlWriter writer) throws Exception {
        ...
        if (this.getForwardAsAttachmentToRecipients().getCount() > 0) {
            this.getForwardAsAttachmentToRecipients()
                    .writeToXml(writer, XmlElementNames.ForwardAsAttachmentToRecipients);
        }
        ...
        if (this.getStopProcessingRules() != false) {
            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.StopProcessingRules,
    ...
    private static EmailAddressCollection convertSMSRecipientsFromMobilePhoneCollectionToEmailAddressCollection(
        ...
        for (MobilePhone recipient : recipientCollection) {
            EmailAddress emailAddress =
                    new EmailAddress(recipient.getName(), recipient.getPhoneNumber(), RuleActions.MobileType);
