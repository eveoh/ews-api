...
abstract class ServiceRequestBase {
     ...
     */
    protected abstract void writeElementsToXml(EwsServiceXmlWriter writer) throws XMLStreamException,
            ServiceXmlSerializationException, ServiceLocalException, InstantiationException, IllegalAccessException,
     ...
     * @throws Exception                 the exception
     */
    protected abstract Object parseResponse(EwsServiceXmlReader reader) throws ServiceXmlDeserializationException,
            XMLStreamException, InstantiationException, IllegalAccessException, ServiceLocalException,
    ...
    protected void throwIfNotSupportedByRequestedServerVersion() throws ServiceVersionException {
        if (this.service.getRequestedServerVersion().ordinal() < this.getMinimumRequiredServerVersion().ordinal()) {
            throw new ServiceVersionException(
                    String.format(Strings.RequestIncompatibleWithRequestVersion, this.getXmlElementName(),
    ...
    /**
     * Gets st ring representation of requested server version. In order to support E12 RTM servers, ExchangeService has
     * another flag indicating that we should use "Exchange2007" as the server version string rather than
     * Exchange2007_SP1.
     *
    ...
    protected HttpWebRequest emit(OutParam<HttpWebRequest> request) throws Exception {
        ...
        if (request.getParam().getResponseCode() >= 400) {
            throw new HttpErrorException(
                    "The remote server returned an error: (" + request.getParam().getResponseCode() + ")" +
                            request.getParam().getResponseText(), request.getParam().getResponseCode());
            // throw new
            // Exception("The remote server returned an error: ("+request.getParam().getResponseCode()+")"+request
            // .getParam().getResponseText());
     ...
     */
    private ByteArrayOutputStream getWebRequestStream(Future request) throws EWSHttpException, InterruptedException,
            ExecutionException {
        ...
        // Reference:
        // http://www.wintellect.com/CS/blogs/jeffreyr/archive/2009/02/08/httpwebrequest-its-request-stream-and
        // -sending-data-in-chunks.aspx
        // return
    ...
    protected void traceResponse(HttpWebRequest request, ByteArrayOutputStream memoryStream) throws XMLStreamException,
        ...
        if (!isNullOrEmpty(contentType) &&
                (contentType.startsWith("text/") || contentType.startsWith("application/soap"))) {
     ...
     */
    protected HttpWebRequest endGetEwsHttpWebResponse(OutParam<HttpWebRequest> request,
                                                      AsyncRequestResult asyncResult) throws Exception {
            ...
            if (request.getParam().getResponseCode() >= 400) {
                throw new HttpErrorException(
                        "The remote server returned an error: (" + request.getParam().getResponseCode() + ")" +
                                request.getParam().getResponseText(), request.getParam().getResponseCode());
                // throw new
                // Exception("The remote server returned an error: ("+request.getParam().getResponseCode()+")
                // "+request.getParam().getResponseText());
