...
public final class AutodiscoverService extends ExchangeServiceBase
    ...
    /**
     * Default implementation of AutodiscoverRedirectionUrlValidationCallback. Always returns true indicating that
     * the URL
     * can be used.
     *
     ...
     */
    private URI getRedirectUrl(String domainName) throws EWSHttpException, XMLStreamException, IOException,
            ServiceLocalException, URISyntaxException {
     ...
     * @throws EWSHttpException   the eWS http exception
     */
    private boolean tryGetRedirectionResponse(HttpWebRequest request, OutParam<URI> redirectUrl) throws
            XMLStreamException, IOException, EWSHttpException {
        // redirectUrl = null;
    ...
    private <TSettings extends ConfigurationSettingsBase> boolean tryLastChanceHostRedirection(Class<TSettings> cls,
                                                                                               ...
                                                                                               OutParam<TSettings>
                                                                                                       settings) throws
            AutodiscoverLocalException, AutodiscoverRemoteException, Exception {
                ...
                try {
                    ...
                    switch (settings.getParam().getResponseType()) {
                        ...
                        case RedirectAddress:
                            ...
                            outParam.setParam(new Integer(currentHop));
                            settings.setParam(
                                    this.internalGetLegacyUserSettings(cls, emailAddress, redirectionEmailAddresses,
                                            outParam));
                        ...
                        default:
                            String failureMessage =
                                    String.format("Autodiscover call at %s failed with error %s, target %s",
                                            redirectionUrl, settings.getParam().getResponseType(),
    ...
    protected GetUserSettingsResponse internalGetLegacyUserSettings(String emailAddress,
        ...
        OutlookConfigurationSettings settings =
                this.getLegacyUserSettings(OutlookConfigurationSettings.class, emailAddress);
    ...
    protected GetUserSettingsResponse internalGetSoapUserSettings(String smtpAddress,
        ...
        for (int currentHop = 0; currentHop < AutodiscoverService.AutodiscoverMaxRedirections; currentHop++) {
            GetUserSettingsResponse response =
                    this.getUserSettings(smtpAddresses, requestedSettings).getTResponseAtIndex(0);
    ...
    private <TGetSettingsResponseCollection, TSettingName> TGetSettingsResponseCollection getSettings(
            ...
            IFunctionDelegate<List<String>, List<TSettingName>, ExchangeVersion, URI,
        ...
        if (this.getRequestedServerVersion().compareTo(MinimumRequestVersionForAutoDiscoverSoapService) < 0) {
            throw new ServiceVersionException(String.format(Strings.AutodiscoverServiceIncompatibleWithRequestVersion,
    ...
    private GetUserSettingsResponseCollection internalGetUserSettings(List<String> smtpAddresses,
                                                                      ...
                                                                      URI autodiscoverUrl) throws ServiceLocalException,
            Exception {
    ...
    protected GetDomainSettingsResponseCollection getDomainSettings(final List<String> domains,
                                                                    ...
                                                                    ExchangeVersion requestedVersion) throws Exception {
			...
			if (this.getCredentials() instanceof WindowsLiveCredentials) {
			   ...
			   else if (this.getCredentials() instanceof PartnerTokenCredentials)
                {
                    if (endpoints.contains( AutodiscoverEndpoints.WSSecuritySymmetricKey))
                    {
                        this.traceMessage(
                            TraceFlags.AutodiscoverConfiguration,
                            String.format("No Autodiscover WS-Security/SymmetricKey endpoint is available for host
                            {0}", host));
                ...
                else if (this.getCredentials()instanceof X509CertificateCredentials)
                {
                    if ((endpoints.contains(AutodiscoverEndpoints.WSSecurityX509Cert))
                    {
                        this.traceMessage(
                            TraceFlags.AutodiscoverConfiguration,
                            String.format("No Autodiscover WS-Security/X509Cert endpoint is available for host {0}",
                            host));
     ...
     * @throws URISyntaxException the URI Syntax exception
     */
    protected List<URI> getAutodiscoverServiceUrls(String domainName, OutParam<Integer> scpHostCount) throws
            URISyntaxException {
        List<URI> urls;
     ...
     */
    protected List<String> getAutodiscoverServiceHosts(String domainName, OutParam<Integer> outParam) throws
    ...
    private boolean tryGetEnabledEndpointsForHost(String host,
        ...
        // We may get redirected to another host. And therefore need to limit the number of redirections we'll tolerate.
     ...
     * @throws EWSHttpException the eWS http exception
     */
    private EnumSet<AutodiscoverEndpoints> getEndpointsFromHttpWebResponse(HttpWebRequest request) throws
            EWSHttpException {
     ...
     * @throws EWSHttpException   the eWS http exception
     */
    protected void traceResponse(HttpWebRequest request, ByteArrayOutputStream memoryStream) throws XMLStreamException,
    ...
    /**
     * Calls the redirection URL validation callback. If the redirection URL validation callback is null,
     * use the default
     * callback which does not allow following any redirections.
     *
     * @param redirectionUrl The redirection URL.
    ...
    private boolean callRedirectionUrlValidationCallback(String redirectionUrl) throws AutodiscoverLocalException {
        IAutodiscoverRedirectionUrl callback =
                (this.redirectionUrlValidationCallback == null) ? this : this.redirectionUrlValidationCallback;
    ...
    protected void processHttpErrorResponse(HttpWebRequest httpWebResponse, Exception webException) throws Exception {
        this.internalProcessHttpErrorResponse(httpWebResponse, webException, TraceFlags.AutodiscoverResponseHttpHeaders,
     ...
     */
    public GetUserSettingsResponse getUserSettings(String userSmtpAddress, UserSettingName... userSettingNames) throws
            Exception {
    ...
    public GetDomainSettingsResponseCollection getDomainSettings(Iterable<String> domains,
                                                                 ExchangeVersion requestedVersion,
                                                                 DomainSettingName... domainSettingNames) throws
            Exception {
        List<DomainSettingName> settings = new ArrayList<DomainSettingName>();
        for (DomainSettingName domainSettingName : domainSettingNames) {
            settings.add(domainSettingName);
