...
public final class ExtendedProperty extends ComplexProperty {
    ...
    protected boolean tryReadElementFromXml(EwsServiceXmlReader reader) throws Exception {
        ...
        if (reader.getLocalName().equals(XmlElementNames.ExtendedFieldURI)) {
            ...
            this.value =
                    MapiTypeConverter.convertToValue(this.getPropertyDefinition().getMapiType(), stringList.iterator());
            return true;
        }
        else {
            return false;
    ...
    protected void writeElementsToXml(EwsServiceXmlWriter writer) throws ServiceXmlSerializationException,
        ...
        if (MapiTypeConverter.isArrayType(this.getPropertyDefinition().getMapiType())) {
            ...
            for (int index = 0; index <= array.size(); index++) {
                writer.writeElementValue(XmlNamespace.Types, XmlElementNames.Value, MapiTypeConverter
                        .convertToString(this.getPropertyDefinition().getMapiType(), array.get(index)));
            }
            writer.writeEndElement();
        }
        else {
            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.Value,
                    MapiTypeConverter.convertToString(this.getPropertyDefinition().getMapiType(), this.getValue()));
        }
    }
    ...
    public void setValue(Object val) throws Exception {
        ...
        if (this.canSetFieldValue(this.value,
                MapiTypeConverter.changeType(this.getPropertyDefinition().getMapiType(), val))) {
    ...
    /**
     * Determines whether the specified <see cref="T:System.Object"/> is equal to the current <see cref="T:System
     * .Object"/>
    ...
    public int hashCode() {
        String printableName =
                this.getPropertyDefinition() != null ? this.getPropertyDefinition().getPrintableName() : "";
