...
public abstract class ServiceObjectSchema implements Iterable<PropertyDefinition> {
    ...
    /**
     * List of all schema types. If you add a new ServiceObject subclass that has an associated schema,
     * add the schema type
     * to the list below.
     */
    private static LazyMember<List<Class<?>>> allSchemaTypes =
            new LazyMember<List<Class<?>>>(new ILazyMember<List<Class<?>>>() {
     ...
     */
    private static LazyMember<Map<String, PropertyDefinitionBase>> allSchemaProperties =
            new LazyMember<Map<String, PropertyDefinitionBase>>(new ILazyMember<Map<String, PropertyDefinitionBase>>() {
                        ...
                        if (null != propertyDefinition.getUri() && !propertyDefinition.getUri().isEmpty()) {
                            ...
                            if (propDefDictionary.containsKey(propertyDefinition.getUri())) {
                                ...
                                for (PropertyDefinition associatedInternalProperty : associatedInternalProperties) {
                                    propDefDictionary
                                            .put(associatedInternalProperty.getUri(), associatedInternalProperty);
                                }
    ...
    public static final PropertyDefinition extendedProperties =
            new ComplexPropertyDefinition<ExtendedPropertyCollection>(ExtendedPropertyCollection.class,
                    XmlElementNames.ExtendedProperty,
                    EnumSet.of(PropertyDefinitionFlags.AutoInstantiateOnRead, PropertyDefinitionFlags.ReuseInstance,
                            PropertyDefinitionFlags.CanSet, PropertyDefinitionFlags.CanUpdate),
                    ExchangeVersion.Exchange2007_SP1, new ICreateComplexPropertyDelegate<ExtendedPropertyCollection>() {
