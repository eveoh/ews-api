...
public final class PropertySet implements ISelfValidate, Iterable<PropertyDefinitionBase> {
            ...
            new LazyMember<Map<BasePropertySet, String>>(new ILazyMember<Map<BasePropertySet, String>>() {
                ...
                public Map<BasePropertySet, String> createInstance() {
                    ...
                    result.put(BasePropertySet.FirstClassProperties,
                            BasePropertySet.FirstClassProperties.getBaseShapeValue());
                    return result;
                }
            });
    /**
     ...
     * @param additionalProperties Additional properties to include in the property set. Property definitions are
     *                             available
     *                             as static members from schema classes (for example, EmailMessageSchema.Subject,
    ...
    /**
     * * Gets type of body that should be loaded on items. If RequestedBodyType is null, body is returned as HTML if
    ...
    /**
     * Sets type of body that should be loaded on items. If RequestedBodyType is null, body is returned as HTML if
     * available, plain text otherwise.
     *
     ...
     */
    protected static void writeAdditionalPropertiesToXml(EwsServiceXmlWriter writer,
                                                         Iterator<PropertyDefinitionBase> propertyDefinitions) throws
    ...
    /**
     * * Validates this property set instance for request to ensure that: 1. Properties are valid for the request server
     * version 2. If only summary properties are legal for this request (e.g. FindItem) then only summary properties
     * were
     * specified.
     *
     ...
     * @throws ServiceValidationException the service validation exception
     */
    protected void validateForRequest(ServiceRequestBase request, boolean summaryPropertiesOnly) throws
            ServiceVersionException, ServiceValidationException {
        for (PropertyDefinitionBase propDefBase : this.additionalProperties) {
            if (propDefBase instanceof PropertyDefinition) {
                ...
                if (propertyDefinition.getVersion().ordinal() >
                        request.getService().getRequestedServerVersion().ordinal()) {
                ...
                if (summaryPropertiesOnly && !propertyDefinition.hasFlag(PropertyDefinitionFlags.CanFind, request.
                    ...
                    throw new ServiceValidationException(String.format("ServiceValidationException :%s,%s,%s",
                            Strings.NonSummaryPropertyCannotBeUsed, propertyDefinition.getName(),
        ...
        if (this.getFilterHtmlContent() != null) {
            if (request.getService().getRequestedServerVersion().compareTo(ExchangeVersion.Exchange2010) < 0) {
                throw new ServiceVersionException(
                        String.format(Strings.PropertyIncompatibleWithRequestVersion, "FilterHtmlContent",
        ...
        if (this.getConvertHtmlCodePageToUTF8() != null) {
            if (request.getService().getRequestedServerVersion().compareTo(ExchangeVersion.Exchange2010_SP1) < 0) {
                throw new ServiceVersionException(
                        String.format(Strings.PropertyIncompatibleWithRequestVersion, "ConvertHtmlCodePageToUTF8",
     ...
     *                            the service xml serialization exception
     */
    protected void writeToXml(EwsServiceXmlWriter writer, ServiceObjectType serviceObjectType) throws
            XMLStreamException, ServiceXmlSerializationException {
        writer.writeStartElement(XmlNamespace.Messages,
                serviceObjectType == ServiceObjectType.Item ? XmlElementNames.ItemShape : XmlElementNames.FolderShape);
        ...
        if (serviceObjectType == ServiceObjectType.Item) {
            ...
            if (this.getFilterHtmlContent() != null) {
                writer.writeElementValue(XmlNamespace.Types, XmlElementNames.FilterHtmlContent,
                        this.getFilterHtmlContent());
            }
            if ((this.getConvertHtmlCodePageToUTF8() != null) && writer.getService().getRequestedServerVersion().
                ...
                writer.writeElementValue(XmlNamespace.Types, XmlElementNames.ConvertHtmlCodePageToUTF8,
