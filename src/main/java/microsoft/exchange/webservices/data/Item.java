...
public class Item extends ServiceObject {
    ...
    /**
     * Initializes an unsaved local instance of <see cref="Item"/>. To bind to an existing item,
     * use Item.Bind() instead.
     *
    ...
    /**
     * Binds to an existing item, whatever its actual type is, and loads the specified set of properties. Calling this
    ...
    protected void internalDelete(DeleteMode deleteMode, SendCancellationsMode sendCancellationsMode,
                                  AffectedTaskOccurrence affectedTaskOccurrences) throws ServiceLocalException,
            Exception {
        this.throwIfThisIsNew();
        this.throwIfThisIsAttachment();
     ...
     * @throws Exception the exception
     */
    protected void internalCreate(FolderId parentFolderId, MessageDisposition messageDisposition,
                                  SendInvitationsMode sendInvitationsMode) throws Exception {
        this.throwIfThisIsNotNew();
        this.throwIfThisIsAttachment();

        if (this.isNew() || this.isDirty()) {
            this.getService().createItem(this, parentFolderId, messageDisposition,
                    sendInvitationsMode != null ? sendInvitationsMode : this.getDefaultSendInvitationsMode());
     ...
     * @throws Exception                the exception
     */
    protected Item internalUpdate(FolderId parentFolderId, ConflictResolutionMode conflictResolutionMode,
                                  MessageDisposition messageDisposition,
                                  SendInvitationsOrCancellationsMode sendInvitationsOrCancellationsMode) throws
        ...
        if (this.isDirty() && this.getPropertyBag().getIsUpdateCallNecessary()) {
            returnedItem = this.getService()
                    .updateItem(this, parentFolderId, conflictResolutionMode, messageDisposition,
    ...
    /**
     * Saves this item in a specific folder. Calling this method results in at least one call to EWS. Mutliple calls
     * to EWS
     ...
     * Drafts folder). Calling this method results in at least one call to EWS. Mutliple calls to EWS might be made if
    ...
    /**
     * Applies the local changes that have been made to this item. Calling this method results in at least one call
     * to EWS.
     ...
     * returns null
     * if the copy operation is across two mailboxes or between a mailbox and a public folder.
     *
    ...
    /**
     * Moves this item to a the specified folder. Calling this method results in a call to EWS. Move returns null if the
     * move operation is across two mailboxes or between a mailbox and a public folder.
     *
     * @param destinationFolderId the destination folder id
     ...
     * @throws Exception the exception
     */
    public void setExtendedProperty(ExtendedPropertyDefinition extendedPropertyDefinition, Object value) throws
            Exception {
     ...
     * request so
     * this item can be property saved or updated.
     *
    ...
    protected boolean getIsTimeZoneHeaderRequired(boolean isUpdateOperation) throws Exception {
        ...
        if (!isUpdateOperation && (this.getService().getRequestedServerVersion().ordinal() >=
            ...
            while (items.hasNext()) {
                ...
                if ((itemAttachment.getItem() != null) &&
                        itemAttachment.getItem().getIsTimeZoneHeaderRequired(false /* isUpdateOperation */)) {
                    return true;
                }
            }
        }
    ...
    /**
     * Gets a value indicating whether this object is a real store item, or if it's a local object that has yet to be
     * saved.
     *
    ...
    public InternetMessageHeaderCollection getInternetMessageHeaders() throws ServiceLocalException {
        return (InternetMessageHeaderCollection) this.getPropertyBag()
                .getObjectFromPropertyDefinition(ItemSchema.InternetMessageHeaders);
    }
    ...
    public EnumSet<ResponseActions> getAllowedResponseActions() throws ServiceLocalException {
        return (EnumSet<ResponseActions>) this.getPropertyBag()
                .getObjectFromPropertyDefinition(ItemSchema.AllowedResponseActions);
    }
    ...
    public ExtendedPropertyCollection getExtendedProperties() throws ServiceLocalException {
        return (ExtendedPropertyCollection) this.getPropertyBag()
                .getObjectFromPropertyDefinition(ServiceObjectSchema.extendedProperties);
    }
    ...
    /**
     * Gets the default setting for how to treat affected task occurrences on Delete. Subclasses will override this for
    ...
    /**
     * Gets the default setting for sending cancellations on Delete. Subclasses will override this for different default
     * behavior.
     *
    ...
    /**
     * Gets the default settings for sending invitations on Save. Subclasses will override this for different default
     * behavior.
     *
    ...
    /**
     * Gets the default settings for sending invitations or cancellations on Update. Subclasses will override this for
     * different default behavior.
     *
