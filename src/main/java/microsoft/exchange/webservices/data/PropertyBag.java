...
class PropertyBag implements IComplexPropertyChanged, IComplexPropertyChangedDelegate {
    ...
    private boolean isRequestedProperty(PropertyDefinition propertyDefinition) {
        ...
        if (this.requestedPropertySet.getBasePropertySet() == BasePropertySet.FirstClassProperties) {
            List<PropertyDefinition> firstClassProps =
                    this.onlySummaryPropertiesRequested ? this.getOwner().getSchema().getFirstClassSummaryProperties() :
     ...
     * @throws ArgumentException
     */
    protected <T> boolean tryGetPropertyType(Class<T> cls, PropertyDefinition propertyDefinition,
        ...
        if (!cls.isAssignableFrom(propertyDefinition.getType())) {
            String errorMessage = String.format(Strings.PropertyDefinitionTypeMismatch,
     ...
     *
     * @param propertyDefinition       The property definition.
     * @param serviceExceptionOutParam Exception that would be raised if there's an error retrieving the property.
     * @return Property value. May be null.
     */
    private Object getPropertyValueOrException(PropertyDefinition propertyDefinition,
        ...
        if (propertyDefinition.getVersion().ordinal() >
                this.getOwner().getService().getRequestedServerVersion().ordinal()) {
            serviceExceptionOutParam.setParam(new ServiceVersionException(
                    String.format(Strings.PropertyIncompatibleWithRequestVersion, propertyDefinition.getName(),
        ...
        if (this.tryGetValue(propertyDefinition, propertyValueOutParam)) {
            ...
            if (propertyDefinition.hasFlag(PropertyDefinitionFlags.AutoInstantiateOnRead)) {
                EwsUtilities.EwsAssert(propertyDefinition instanceof ComplexPropertyDefinitionBase,
                        "PropertyBag.get_this[]", "propertyDefinition is " +
                ...
                if (propertyDefinition != this.getOwner().getIdPropertyDefinition()) {
                    ...
                    if (!propertyDefinition.isNullable()) {
                        String errorMessage =
                                this.isRequestedProperty(propertyDefinition) ? Strings.ValuePropertyNotLoaded :
    ...
    /**
     * Sets the isDirty flag to true and triggers dispatch of the change event to the owner of the property bag. Changed
     * must be called whenever an operation that changes the state of this property bag is performed (e.g. adding or
     * removing a property).
     */
     ...
     * @param clear                          Indicates whether the bag should be cleared before properties are loaded.
     ...
     */
    protected void loadFromXml(EwsServiceXmlReader reader, boolean clear, PropertySet requestedPropertySet,
        ...
        try {
            do {
                ...
                if (reader.getNodeType().getNodeType() == XMLNodeType.START_ELEMENT) {
                    ...
                    if (this.getOwner().schema()
                            .tryGetPropertyDefinition(reader.getLocalName(), propertyDefinitionOut)) {
    ...
    /**
     * Initializes a ComplexProperty instance. When a property is inserted into the bag, it needs to be initialized in
     * order for changes that occur on that property to be properly detected and dispatched.
     *
     ...
     * @throws Exception the exception
     */
    private void writeSetUpdateToXml(EwsServiceXmlWriter writer, PropertyDefinition propertyDefinition) throws
            Exception {
        // The following test should not be necessary since the property bag
        // prevents
     ...
     * @throws Exception the exception
     */
    private void writeDeleteUpdateToXml(EwsServiceXmlWriter writer, PropertyDefinition propertyDefinition,
     ...
     * @throws ServiceLocalException ServiceVersionException will be raised if this property requires a later version of
     *                               Exchange. ServiceObjectPropertyException will be raised for get if property hasn't
     *                               been assigned or loaded, raised for set if property cannot be updated or deleted.
     */
    protected Object getObjectFromPropertyDefinition(PropertyDefinition propertyDefinition) throws
     ...
     * @throws Exception the exception
     */
    protected void setObjectFromPropertyDefinition(PropertyDefinition propertyDefinition, Object object) throws
            Exception {
        if (propertyDefinition.getVersion().ordinal() >
                this.getOwner().getService().getRequestedServerVersion().ordinal()) {
        ...
        if (!this.loading) {
            ...
            if (this.getOwner().isNew() && !propertyDefinition.hasFlag(PropertyDefinitionFlags.CanSet,
                    this.getOwner().getService().getRequestedServerVersion())) {
