...
class EwsUtilities {
    ...
    private static LazyMember<ServiceObjectInfo> serviceObjectInfo =
            new LazyMember<ServiceObjectInfo>(new ILazyMember<ServiceObjectInfo>() {
   ...
   /* 	InputStream inputStream = new FileInputStream ("D:\\EWS ManagedAPI sp2\\Rp\\xml\\useravailrequest.xml");
            ...
            new LazyMember<Map<Class<?>, Map<String, ExchangeVersion>>>(
                    new ILazyMember<Map<Class<?>, Map<String, ExchangeVersion>>>() {
                        ...
                        public Map<Class<?>, Map<String, ExchangeVersion>> createInstance() {
                            ...
                            enumDicts
                                    .put(MeetingRequestsDeliveryScope.class, buildEnumDict(MeetingRequestsDeliveryScope.
                                            class));
                            return enumDicts;
                        }
                    });
    ...
    private static LazyMember<Map<Class<?>, Map<String, String>>> schemaToEnumDictionaries =
            new LazyMember<Map<Class<?>, Map<String, String>>>(new ILazyMember<Map<Class<?>, Map<String, String>>>() {
                @Override
    ...
    private static LazyMember<Map<String, String>> typeNameToShortNameMap =
            new LazyMember<Map<String, String>>(new ILazyMember<Map<String, String>>() {
     ...
     */
    protected static <TServiceObject extends ServiceObject> TServiceObject createEwsObjectFromXmlElementName(
        ...
        if (EwsUtilities.serviceObjectInfo.getMember().getXmlElementNameToServiceObjectClassMap()
                .containsKey(xmlElementName)) {
            itemClass = EwsUtilities.serviceObjectInfo.getMember().getXmlElementNameToServiceObjectClassMap()
                    .get(xmlElementName);
            if (EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithServiceParam()
                    .containsKey(itemClass)) {
                creationDelegate =
                        EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithServiceParam()
                                .get(itemClass);
     ...
     * @throws Exception the exception
     */
    protected static Item createItemFromItemClass(ItemAttachment itemAttachment, Class itemClass, boolean isNew) throws
            Exception {
        ICreateServiceObjectWithAttachmentParam creationDelegate;
        if (EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithAttachmentParam()
                .containsKey(itemClass)) {

            creationDelegate =
                    EwsUtilities.serviceObjectInfo.getMember().getServiceObjectConstructorsWithAttachmentParam()
                            .get(itemClass);
     ...
     * @throws Exception the exception
     */
    protected static Item createItemFromXmlElementName(ItemAttachment itemAttachment, String xmlElementName) throws
            Exception {
        Class<?> itemClass;
        if (EwsUtilities.serviceObjectInfo.getMember().getXmlElementNameToServiceObjectClassMap()
                .containsKey(xmlElementName)) {
    ...
    protected static Class getItemTypeFromXmlElementName(String xmlElementName) {
        ...
        return EwsUtilities.serviceObjectInfo.getMember().getXmlElementNameToServiceObjectClassMap().get(xmlElementName)
                .getClass();

    }

    /**
     * Finds the first item of type TItem (not a descendant type) in the specified collection. <typeparam
     * name="TItem">The
     * type of the item to find.</typeparam>
     *
     ...
     */
    private static void writeTraceStartElement(XMLStreamWriter writer, String traceTag, boolean includeVersion) throws
            XMLStreamException {
     ...
     */
    protected static String formatHttpRequestHeaders(HttpWebRequest request) throws URISyntaxException,
            EWSHttpException {
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("%s %s HTTP/%s\n", request.getRequestMethod().toUpperCase(),
                request.getUrl().toURI().getPath(), "1.1"));
     ...
     */
    protected static <T extends Enum> void parseEnumValueList(Class<T> c, List<T> list, String value,
                                                              char... separators) {
     ...
     */
    protected static <T> T parse(Class<T> cls, String value) throws InstantiationException, IllegalAccessException,
            ParseException {
    ...
    /**
     * Takes a System.TimeSpan structure and converts it into an xs:duration string as defined by the W3 Consortiums
     * Recommendation "XML Schema Part 2: Datatypes Second Edition", http://www.w3.org/TR/xmlschema-2/#duration
     *
    ...
    protected static String getTimeSpanToXSDuration(TimeSpan timeOffset) {
        ...
        return String
                .format("%sP%sDT%sH%sM%sS", offsetStr, Math.abs(timeOffset.getDays()), Math.abs(timeOffset.getHours()),
                        Math.abs(timeOffset.getMinutes()),
                        Math.abs(timeOffset.getSeconds()) + "." + Math.abs(timeOffset.getMilliseconds()));
    }

    /**
     * Takes an xs:duration string as defined by the W3 Consortiums Recommendation "XML Schema Part 2: Datatypes Second
     * Edition", http://www.w3.org/TR/xmlschema-2/#duration, and converts it into a System.TimeSpan structure This
     * method
     * uses the following approximations: 1 year = 365 days 1 month = 30 days Additionally,
     * it only allows for four decimal
     * points of seconds precision.
     *
...
//		long retval=1010010;
    ...
    public static String timeSpanToXSTime(TimeSpan timeSpan) {
        ...
        return String
                .format("%s:%s:%s", myFormatter.format(timeSpan.getHours()), myFormatter.format(timeSpan.getMinutes()),
     ...
     */
    protected static void validateNonBlankStringParamAllowNull(String param, String paramName) throws ArgumentException,
     ...
     * @throws ServiceLocalException
     */
    protected static void validateNonBlankStringParam(String param, String paramName) throws ArgumentNullException,
            ArgumentException, ServiceLocalException {
        if (param == null) {
     ...
     */
    protected static void validateEnumVersionValue(Enum<?> enumValue, ExchangeVersion requestVersion) throws
        ...
        if (enumVersionDict.containsKey(enumValue.toString())) {
            ...
            if (i < 0) {
                throw new ServiceVersionException(
                        String.format("%S,%S,%S,%S", Strings.EnumValueIncompatibleWithRequestVersion,
     ...
     */
    protected static void validateServiceObjectVersion(ServiceObject serviceObject,
        ...
        if (requestVersion.ordinal() < minimumRequiredServerVersion.ordinal()) {
            String msg =
                    String.format(Strings.ObjectTypeIncompatibleWithRequestVersion, serviceObject.getClass().getName(),
     ...
     * @throws ServiceVersionException The service version exception
     */
    protected static void validatePropertyVersion(ExchangeService service, ExchangeVersion minimumServerVersion,
     ...
     * @throws ServiceVersionException the service version exception
     */
    protected static void validateMethodVersion(ExchangeService service, ExchangeVersion minimumServerVersion,
     ...
     */
    protected static void validateClassVersion(ExchangeService service, ExchangeVersion minimumServerVersion,
     ...
     * @throws ServiceLocalException
     */
    protected static int countMatchingChars(String str, IPredicate<Character> charPredicate) throws
            ServiceLocalException {
        int count = 0;
     ...
     * @return True if every element in the collection matches the conditions defined by the specified predicate;
     *         otherwise, false.
     * @throws ServiceLocalException
     */
    protected static <T> boolean trueForAll(Iterable<T> collection, IPredicate<T> predicate) throws
            ServiceLocalException {
