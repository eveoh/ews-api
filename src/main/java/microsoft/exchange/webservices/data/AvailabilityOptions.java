...
public final class AvailabilityOptions {
    ...
    protected void validate(long timeWindow) throws Exception {
        if (this.mergedFreeBusyInterval > timeWindow) {
            throw new IllegalArgumentException(
                    String.format("%s,%s", Strings.MergedFreeBusyIntervalMustBeSmallerThanTimeWindow,
    ...
    protected void writeToXml(EwsServiceXmlWriter writer, GetUserAvailabilityRequest request) throws Exception {
        if (request.isFreeBusyViewRequested()) {
            ...
            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.MergedFreeBusyIntervalInMinutes,
        ...
        if (request.isSuggestionsViewRequested()) {
            ...
            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.MaximumResultsByDay,
                    this.maximumSuggestionsPerDay);

            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.MaximumNonWorkHourResultsByDay,
                    this.maximumNonWorkHoursSuggestionsPerDay);

            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.MeetingDurationInMinutes,
                    this.meetingDuration);

            writer.writeElementValue(XmlNamespace.Types, XmlElementNames.MinimumSuggestionQuality,
                    this.minimumSuggestionQuality);

            TimeWindow timeWindowToSerialize =
                    this.detailedSuggestionsWindow == null ? request.getTimeWindow() : this.detailedSuggestionsWindow;
            ...
            if (this.currentMeetingTime != null) {
                writer.writeElementValue(XmlNamespace.Types, XmlElementNames.CurrentMeetingTime,
    ...
    /**
     * Gets the time difference between two successive slots in a FreeBusyMerged view. MergedFreeBusyInterval must be
     * between 5 and 1440. The default value is 30.
     *
    ...
    public void setMergedFreeBusyInterval(int value) {
        if (value < 5 || value > 1440) {
            throw new IllegalArgumentException(
                    String.format("%s,%s,%s,%s", Strings.InvalidPropertyValueNotInRange, "MergedFreeBusyInterval", 5,
                            1440));
        }
    ...
    /**
     * Gets  the percentage of attendees that must have the time period open for the time period to qualify as a good
     * suggested meeting time. GoodSuggestionThreshold must be between 1 and 49. The default value is 25.
     *
    ...
    /**
     * Gets the number of suggested meeting times that should be returned per day. MaximumSuggestionsPerDay must be
     * between
     * 0 and 48. The default value is 10.
     *
    ...
    public void setMaximumSuggestionsPerDay(int value) {
        if (value < 0 || value > 48) {
            throw new IllegalArgumentException(
                    String.format("%s,%s,%s,%s", Strings.InvalidPropertyValueNotInRange, "MaximumSuggestionsPerDay", 0,
                            48));
        }
    ...
    public void setMaximumNonWorkHoursSuggestionsPerDay(int value) {
        if (value < 0 || value > 48) {
            throw new IllegalArgumentException(
                    String.format(Strings.InvalidPropertyValueNotInRange, "MaximumNonWorkHoursSuggestionsPerDay", 0,
                            48));
        }
     ...
     * between 30
     * and 1440. The default value is 60.
     *
    ...
    public void setMeetingDuration(int value) {
        if (value < 30 || value > 1440) {
            throw new IllegalArgumentException(
                    String.format("%s,%s,%s,%s", Strings.InvalidPropertyValueNotInRange, "MeetingDuration", 30, 1440));
        }
