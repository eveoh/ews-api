...
class HangingTraceStream extends InputStream {
     ...
     *
     * @param buffer An array of bytes. When this method returns, the buffer contains the specified byte array with the
     *               values between
     * @param offset The zero-based byte offset in at which to begin storing the data read from the current stream.
     * @param count  The maximum number of bytes to be read from the current stream.
     * @return The total number of bytes read into the buffer. This can be less than the number of bytes requested if
     * that
    ...
    public int read(byte[] buffer, int offset, int count) throws IOException {
        ...
        if (HangingServiceRequestBase.LogAllWireBytes) {
            ...
            String logMessage = String.format("HangingTraceStream ID [%d] " + "returned %d bytes. Bytes returned: [%s]",
                    this.hashCode(), retVal, readString);
