...
abstract class MultiResponseServiceRequest<TResponse extends ServiceResponse> extends SimpleServiceRequestBase {
    ...
    protected Object parseResponse(EwsServiceXmlReader reader) throws Exception {
        ...
        if (serviceResponses.getCount() < this.getExpectedResponseMessageCount()) {
            if ((serviceResponses.getCount() == 1) &&
                    (serviceResponses.getResponseAtIndex(0).getResult() == ServiceResult.Error)) {
                throw new ServiceResponseException(serviceResponses.getResponseAtIndex(0));
            }
            else {
                throw new ServiceXmlDeserializationException(
                        String.format(Strings.TooFewServiceReponsesReturned, this.getResponseMessageXmlElementName(),
                                this.getExpectedResponseMessageCount(), serviceResponses.getCount()));
            }
        }
     ...
     * @throws Exception
     */
    protected MultiResponseServiceRequest(ExchangeService service, ServiceErrorHandling errorHandlingMode) throws
            Exception {
        super(service);
