...
class SoapFaultDetails {
    ...
    /**
     * Dictionary of key/value pairs from the MessageXml node in the fault. Usually empty but there are a few cases
     * where
     * SOAP faults may include MessageXml details (e.g. CASOverBudgetException includes BackoffTime value).
     */
     ...
     * @throws Exception          the exception
     */
    private void parseDetailNode(EwsXmlReader reader) throws ServiceXmlDeserializationException, XMLStreamException,
            Exception, Exception {
        do {
            reader.read();
            if (reader.getNodeType().equals(new XMLNodeType(XMLNodeType.START_ELEMENT))) {
            ...
            do {
                ...
                if (reader.isStartElement() && !reader.isEmptyElement()) {
                    ...
                    if (localName.equals(XmlElementNames.Value)) {
                        this.errorDetails
                                .put(reader.readAttributeValue(XmlAttributeNames.Name), reader.readElementValue());
                    }
                }
